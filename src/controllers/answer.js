...
class Answer {
   ...
   * @returns {Object} response - response object
   */
  static downvoteComment(request, response, next) {
    const { answerId } = request.params;
    const { questionId } = request.params;
    const downvoteCount = parseInt(request.body.count, 10);
          ...
          db.query(`SELECT * FROM answers where id = ${answerId}`)
            .then((result1) => {
              if (result1.rows < 1) {
                ...
                db.query(`UPDATE answers SET downvote = ${downvoteCount + 1} 
                WHERE id = ${answerId}`)
                  .then(() => response.status(200).json({
                    status: 'Success',
                    message: 'Downvote was successful'
                  }))
                  .catch(error => response.status(500).json({
                    status: 'Failure',
                    message: 'Internal server error'
